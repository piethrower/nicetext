...
| `whats-new.html` | Modern academic paper: sidebar TOC, anchor-linked bibliography, working figures (expansion-rate tables, same-payload-eight-styles panel, SIZER/DESIZER walk-through). | ⬜ queued |
...
runs the encode/decode pipeline entirely in the browser via the worker
jobs API (see `docs/architecture-workers.md`). Imports
`./src/index.js` directly. Two tabs (Encode / Decode), two
dropdowns on the encode side:

- **Dictionary**: mit / jfk / aesop / wizoz / shakespeare / master
- **Style source** (flat (random words) / CFG grammar / "X style) random pick" or "X style, sequential" for each corpus model table

Picking a style with a `requireDict` auto-switches the dict; the page
shows a hint if you override. UTF-8 sniff on recover so binary files
prompt download instead of garbling the textarea.

Run locally with `tools/serve.sh` (python3 http.server on :8888 from
repo root). Static origin is required because `fetch()` blocks `file://`
in modern browsers.

Currently does NOT include: custom dict/grammar/wordlist upload,
in-browser dictionary builder, sample-text editor. The builder code
(`js/src/builder/*` + `js/src/grammar/*` + `js/src/builder/genmodel.js`)
is already browser-safe; the page can be extended without server work.
...
- **Repo restructure to Span-It! conventions** (`9bc9db6`).
  ...
  `tests/node/`. `npm test` now runs `node --test tests/node/`.
...
Builds on the tabs/navbar pass below. Three threads:
