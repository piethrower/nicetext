...
HTML page that does encode/decode entirely in the browser. Source
preserved at `OG-NiceText-C++/` for reference; **read-only by rule**.
...
| `begins_with_a_vowel` | auto-generated by `applyVowelAugmentation` (sources.js) when `base.augment.vowel` is true in the byos.json |
| `claude2026`         | `fixture-src/twlist/claude2026/claude2026.twlist` |

The OG `doc/distribution.txt` (in `OG-NiceText-C++/nicetext-1.0/doc/`)
documents the upstream provenance of the first six.
...
- **No statistical analysis of detectability.** The thesis acknowledges
  ...
  port.
- **No async streaming I/O.** The wire format supports streaming, but
  encode/decode currently materialize the payload as a `Uint8Array`. A
  generator-based async path is a clean follow-on.
- **No web upload of custom dicts/grammars.** Architecture supports
  this (the catalog in `js/app.js` is just an array; the builder code
  in `js/src/builder/` is browser-safe), but the UI work hasn't been
  done.
...
- Mark T. Chapman, "Hiding the Hidden: A Software System for Concealing
  ...
  Preserved in `OG-NiceText-C++/nicetext-0.9/doc/thesis.txt`.
- M. Chapman & G. Davida, "Hiding the Hidden", *Proc. ICICS 1997*,
  Beijing, Springer LNCS. Preserved in
  `OG-NiceText-C++/nicetext-0.9/doc/icics97.txt`.
...
(`fixtures/norvig.freq.tsv.gz`, `fixtures/google-books.freq.tsv.gz`,
`fixtures/gutenberg.freq.tsv.gz`) covering 70% of the active 418K-word
vocab pool. Reproducers under `fixture-src/freq/<source>/`. Engine
